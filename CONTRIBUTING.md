# 貢獻指南
以下貢獻指南供所有本專案的開發者與 AI 進行參考，以確保代碼的可維護性。
...
每個 commit 應代表一個獨立且完整的邏輯變更，**main 分支**上的任意 commit 皆可正常編譯與運作。

- **一個 commit，一件事**：新增模組、整合模組、修 bug 應分開提交，不混在一起
- **不可拆的例外**：若兩個變更缺一不可才能編譯（例如新增模組 + CMakeLists 註冊），可合併為一個 commit
- **WIP 分支**：跨開發機同步未完成的程式碼請使用 `wip/<GitHub 名稱>` 分支，不受原子提交限制
